...
def exibir_historico(request):
    historicos = HistoricoTeste.objects.all()  # Obtenha todos os registros do histórico
    return render(request, 'core/historico_testes.html', {'historicos': historicos})
...
@login_required
def historico_testes(request):
    historico_testes = HistoricoTeste.objects.filter(usuario=request.user)
    return render(request, 'historico_testes.html', {'historico_testes': historico_testes})
